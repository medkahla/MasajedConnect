import { DataService, Mosque, PrayerTimes } from '../types';
...
export const FirebaseDataService: DataService = {
  getMosques: async () => {
    ...
    return snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Mosque));
  },
  ...
  getMosqueById: async (id: string) => {
    ...
    return snap.exists() ? ({ id: snap.id, ...snap.data() } as Mosque) : undefined;
  },
  ...
  login: async () => {
    ...
    console.warn("Direct login via service is not standard for Firebase. Use AuthContext.");
    return null;
  }
};
